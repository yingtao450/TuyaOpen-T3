...
void bignum_print(char *prefix, const mbedtls_mpi *R)
{
	...
	os_printf("%s: %s0x", prefix, R->s == -1 ? "-" : "");

	for (i = R->n - 1; i >= 0; i--)
		os_printf("%08lx", R->p[i]);
		...
		const struct crypto_ec_point *point, u8 *x, u8 *y)
{
	...
	if (x) {
		if(crypto_bignum_to_bin(
			(struct crypto_bignum *)&((mbedtls_ecp_point *)point)->X,
					x, len, len) < 0) {
			return -1;
		}
	...
	if (y) {
		if(crypto_bignum_to_bin(
			(struct crypto_bignum *)&((mbedtls_ecp_point *)point)->Y,
					y, len, len) < 0) {
			return -1;
		}
	}
		...
		const struct crypto_ec_point *p)
{
	...
	MBEDTLS_MPI_CHK(mbedtls_mpi_exp_mod(&y_sqr_lhs,
			&((const mbedtls_ecp_point *)p)->Y , &two, &e->group.P, NULL));

	y_sqr_rhs = (mbedtls_mpi *) crypto_ec_point_compute_y_sqr(e,
		(const struct crypto_bignum *) & ((const mbedtls_ecp_point *)p)->X);
		...
		const u8 *buf, size_t len)
{
	...
	mbedtls_ecp_group_load_ext(&mbedtls_pk_ec(*key)->grp, MBEDTLS_ECP_DP_SECP256R1);
...
#else
	int len = ecdh->ctx.ctx.mbed_ecdh.Q.X.n * 2;
#endif
	int ret;
...
#else
	ret = mbedtls_mpi_write_binary(&ecdh->ctx.ctx.mbed_ecdh.Q.X, wpabuf_put(buf, len), len);
#endif
	if (ret != 0) {
									   ...
									   const u8 *key, size_t len)
{
	...
	peerKey = &ecdh->ctx.Qp;
#else
	peerKey = &ecdh->ctx.ctx.mbed_ecdh.Qp;
#endif
...
#else
	if (crypto_ec_point_solve_y_coord((void *)&ecdh->ctx.ctx.mbed_ecdh.grp,
		(void *)peerKey, (void *)&peerKey->X, 0)) {
		//wpa_printf(MSG_ERROR, "OWE: Fail to calc Y using X(y^2 = x^3 + ax + b mod P)");
		goto clean;
	}
#endif
