...
bk_err_t bk_wifi_get_ip_status_internal(IPStatusTypedef *outNetpara, WiFi_Interface inInterface);
...
void sta_ip_mode_set(int dhcp)
{
	if (dhcp == 1) {
		...
		ret = bk_wifi_get_ip_status_internal(&ipStatus, BK_STATION);
		if (ret == kNoErr)
