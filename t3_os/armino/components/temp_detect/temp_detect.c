...
extern uint64_t sys_hal_get_exit_low_voltage_tick(void);
...
#define WAKEUP_TICK_THRES_LOW_COMPENSATION    120 //120seconds
#define WAKEUP_TICK_THRES_HIGH_COMPENSATION   30   // 30seconds
uint16_t tempd_adjust_temperature(uint16_t temp_code)
{
#if (CONFIG_SOC_BK7236XX)
	uint64_t previous_tick = sys_hal_get_exit_low_voltage_tick();
	uint64_t current_tick  = bk_aon_rtc_get_current_tick(AON_RTC_ID_1);
	uint32_t diff_second;
	uint32_t diff_code;

	if (current_tick < previous_tick) {
		diff_second = 0xFFFFFFFF;
	} else {
		diff_second = (current_tick - previous_tick) / bk_rtc_get_ms_tick_count() / 1000;
	}

	/**
	 * f(0)=-23
	 * f(30)=-5 ==> f(t)=0.6t-23    [0,30]
	 * f(120)=0 ==> f(t)=(t-120)/18 [30,120]
	 */
	if (diff_second <= WAKEUP_TICK_THRES_HIGH_COMPENSATION) {
		diff_code = 23 - 3 * diff_second / 5;
	} else if (diff_second <= WAKEUP_TICK_THRES_LOW_COMPENSATION) {
		diff_code = (120 - diff_second) / 18;
	} else {
		diff_code = 0;
	}

	if (temp_code > diff_code) {
		temp_code -= diff_code;
	} else {
		temp_code = 0;
	}
...
static int tempd_adc_get_temperature(uint16_t *temperature)
{
	...
	*temperature = tempd_adjust_temperature(*temperature);
	//TEMPD_LOGW("%d=>%d\r\n", s_raw_temperature_data[0], *temperature);
