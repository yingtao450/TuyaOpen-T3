...
#define QSPI_DATA_LL_REG_BASE               (SOC_QSPI0_DATA_BASE)

#define QSPI_LL_CLK_PIN     GPIO_22
#define QSPI_LL_CSN_PIN     GPIO_23
#define QSPI_LL_IO0_PIN     GPIO_24
#define QSPI_LL_IO1_PIN     GPIO_25
#define QSPI_LL_IO2_PIN     GPIO_26
#define QSPI_LL_IO3_PIN     GPIO_27
...
static inline void qspi_ll_direct_write(uint32_t base_addr, const void *data, uint32_t size)
{
	for (int i = 0; i < size; i++) {
		*((uint8_t *)(QSPI_DATA_LL_REG_BASE + base_addr + i)) = *((uint8_t *)data + i);
	}
}
...
{
	for (int i = 0; i < size; i++) {
		*((uint8_t *)data + i) = *((uint8_t *)(QSPI_DATA_LL_REG_BASE + base_addr + i));
	}
}
