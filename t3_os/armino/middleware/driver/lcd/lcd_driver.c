...
#if defined(TUYA_LCD_BL_CTRL_MODE)
#if (TUYA_LCD_BL_CTRL_MODE == BL_CTRL_GPIO)
#define LCD_BACKLIGHT_GPIO          TUYA_LCD_BACKLIGHT_GPIO
#elif (TUYA_LCD_BL_CTRL_MODE == BL_CTRL_PWM)
// TODO
// #define LCD_BACKLIGHT_PWM		TUYA_LCD_BACKLIGHT_PWM
#else
#error No defined lcd back light mode.
#endif
#else // !TUYA_LCD_BL_CTRL_MODE
...
int32_t lcd_driver_get_spi_gpio(LCD_SPI_GPIO_TYPE_E gpio_type)
{
    ...
    switch(gpio_type)
    {
        ...
        case SPI_GPIO_RST:
            // Modified by TUYA Start
            #ifdef TUYA_LCD_SPI_RST
            gpio_value = TUYA_LCD_SPI_RST;
            #else
            if(s_lcd.device.id == LCD_DEVICE_ST7701S)
                gpio_value = GPIO_15;
            else
                gpio_value = GPIO_6;
            #endif // TUYA_LCD_SPI_RST
            // Modified by TUYA End
            break;
        default:
...
#elif (CONFIG_SOC_BK7236XX)
// Modified by TUYA Start
#if CONFIG_PWM && defined(TUYA_LCD_BL_CTRL_MODE) && (TUYA_LCD_BL_CTRL_MODE == BL_CTRL_PWM)
    BK_LOG_ON_ERR(bk_pwm_driver_init());
    pwm_init_config_t config = {0};
    config.period_cycle = 100;
    config.duty_cycle = 100;
...
#endif // CONFIG_PWM && !CONFIG_TUYA_ST7701S
// Modified by TUYA End
#endif
