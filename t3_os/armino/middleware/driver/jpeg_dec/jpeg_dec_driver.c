...
bool jpeg_dec_line_size_match(uint8_t *src, uint32_t src_size, uint32_t dec_size)
{
	bool ok = false;
    ...
    if (jpeg_dec_hal_get_jpeg_dec_linen())  //line num int en
    {
        if (!jpeg_dec_hal_get_mcu_index())
        {
...
static void jpeg_decoder_isr(void)
{
	...
	if (jpeg_dec_ll_get_reg0x5f_dec_frame_int_clr()) {
...
#endif
#else  //BK7258
        result.ok = jpeg_dec_line_size_match(jpeg_address, jpeg_size, result.size);
#endif
		...
		LOGD("int status = %x not auto int and line int \r\n", jpeg_dec_hal_get_int_status_value());
	}
}
